...
class ReviewerAgent(AssistantAgent):
    ...
    _system_message_new = textwrap.dedent(
        ...
        - If you do not approve the code, you should give constructive feedback and comments on how to make the code
          better.
        - If you approve the code, which means there are no issues to be fixed or improvements to be made, simply
          say 'CODE APPROVED' without any other content.
        - You can insist multiple review rounds if you find issues in the code in your review.

        ## Constraints
        - If you do not approve the code, do not include 'CODE APPROVED' to your answer.
        """
    )
    ...
    _system_message_modify = textwrap.dedent(
        ...
        ## Instructions
        - When reviewing the code changes, compare the changes with the existing code files.
        - Ensure that the code changes are aligned with the existing code base and are in the scope of the requested
          changes.
        - Verify that the architecture is solid and follows good design principles.
        - Verify that the code follows good software design principles and patterns, such as SOLID principles, DRY,
          KISS, and YAGNI.
